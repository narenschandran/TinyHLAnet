...
def HLA_length_aatype_position_num(hla = False, length = 9, label = None):
    ...
    try:
        aatype_position = np.load(os.path.join(SCRIPT_DIR, './Attention/peptideAAtype_peptidePosition/{}_Length{}.npy'.format(hla, length)),
                                  allow_pickle = True).item()[new_label]

        aatype_position_num = np.load(os.path.join(SCRIPT_DIR, './Attention/peptideAAtype_peptidePosition_NUM/{}_Length{}_num.npy'.format(hla, length)),
                                       allow_pickle = True).item()[new_label]
    except:
        print('No {} with {}, Use the overall attention for pepAAtype-peppsition'.format(hla, length))
        aatype_position = np.load(os.path.join(SCRIPT_DIR, './Attention/peptideAAtype_peptidePosition/Allsamples_Alllengths.npy'),
                                  allow_pickle = True).item()[length][new_label]
        aatype_position_num = np.load(os.path.join(SCRIPT_DIR, './Attention/peptideAAtype_peptidePosition_NUM/Allsamples_Alllengths_num.npy'),
                                      allow_pickle = True).item()[length][new_label]
